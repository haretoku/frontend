...
test("6つの詳細ページが共通構造を持つ", async () => {
  for (const pageName of pageNames) {
    const html = await readFile(new URL(`../pages/${pageName}`, import.meta.url), "utf8");
    assert.match(html, /<h2>結論<\/h2>/);
    assert.match(html, /\.\.\/assets\/css\/article\.css/);
    assert.match(html, /\.\.\/assets\/js\/article-data\.js/);
