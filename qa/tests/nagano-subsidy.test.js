...
test('長野住宅別枝・10kWh境界・控除前費用・排他・非現金を保持する',()=>{
  ...
  const k=standard('20210').included_programs.find(p=>p.government_level==='municipality');assert.equal(k.amount_yen,50000);assert.match(noncashBenefitDescription(k),/非現金ポイント/);
});
