...
test("結果は主要結論，見積もり導線，段階的開示の順である", async () => {
  ...
  assert.match(app, /renderCashflowAmount\(elements\.resultGrossCost, grossInstallationCost, "cost"\)/);
  ...
  assert.match(app, /renderCashflowAmount\(elements\.capacityInitialCost, netInitialOutlay, "cost"\)/);
  assert.match(app, /renderCashflowAmount\(elements\.capacityProfit, selectedScenario\.profit_yen\)/);
  assert.match(app, /cashflow-amount--neutral/);
