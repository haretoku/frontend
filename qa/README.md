...
`pnpm run test:public`はfrontend単体で公開計算・記事・サイト・入力・ガイド・GA4を検査する．`pnpm run test:local-review`は隣接backendの監査原本・投影を利用する自治体・都道府県監査UIの検査であり，Pages公開ジョブでは実行しない．`pnpm test`は両方を実行し，失敗を無視しない．公開用fixtureはGit LFSで本体を取得する．QA用ファイルは公開サイトの実行時処理から参照しない．
...
## Pages公開検査とローカル監査QAの分離（v0.1.1）

> **結論：公開用6検査ファイルをfrontend単体で実行し，backend監査原本を必要とする2検査ファイルはローカル専用コマンドで維持する．**
...
## 2026-09-25 公開前URL・導線・文字表記の修正

> **結論：e04553f後の未コミット差分で公開テスト1,105件とPages検証はPASS，独立検収・HTTPS・リリースは統括へ引き渡す．**

対象はガイド一覧と4記事の/guides/移転，旧URLの静的転送，運営者名，未接続見積もり導線の非表示，TOP電気代補足，ナビ表記及び通常文字の強調解除．計算・受領データ11.17.0は変更していない．公開用テスト6ファイル指定で1,105件PASS（失敗0），Pages build/finalize/verifyは70ファイル・正規9URL・公開データ原バイト一致・CSS順序・参照資産・開発資料非混入を確認した．旧URLはlocation.replaceと明示リンクで検索条件・ハッシュを保持し，HTTP 301ではない．旧ガイドURLでGA4が動かないこともテストした．

自己実画面確認は公開用4173のPC1280／360 px診断（佐賀41，既存，4 kW，標準，約62万円・約23年），TOP，ガイド，収支記事，及び360 pxの補助金・見積もり・停電記事で行った．通常幅1265／345 pxで横はみ出しなし，見積もりバー0件・記事内カードdisplay:none．ロゴはヘッダー／フッター，TOPキャッチ・診断タイトル・「知って，なっトク」，診断見出し，ガイド見出しの既存brand-word装飾を維持する．通常のbrand-termは周囲のNoto Sans JP・太さ・色を継承し，収支記事パンくずで400，ナビで700を確認した．

最終点検で移転時のmeta contentへの余計なパス付与を修正し，viewport・OGP・referrerを自動検査へ追加して全1,105件を再実行した．この最終メタ情報修正後のブラウザー再読込はCDP応答待ちとなり，再確認は完了していない．それ以前のPC／360 px表示確認と区別し，統括の独立検収で修正版を確認する．検査用4173は停止し，一時viewportを解除・検査タブを閉じた．ユーザー指示の比較用5173（統括起動PID32792）は保持．commit／push／deployなし，版番号は0.1.2を保持し，次版採番とHTTPS確認・公開は統括が担当する．

### 最終スマホ再確認と転送告知の追加修正

統括からの再確認依頼で公開用4173を再起動し，最終メタ情報版のガイドを新規タブで読み込めることを確認した．ただし360 px指定後のreloadは完了しても，続くPage.getFrameTree／Runtime.evaluateがtimeoutとなり，最終幅・画像は取得できなかった．viewport解除は成功，検査タブのcloseもEmulation.setFocusEmulationEnabledでtimeoutとなったため，タブは自動終了対象のままとした．最終スマホ検査の阻害は解消していない．共有5173は保持した．

ユーザー追加指示により，旧一覧・4記事の「移転のお知らせ」と通常表示の本文を除去し，noscript内の最小リンクだけを残した．転送JSはheadのcanonicalから移動先を取得するため，JavaScript有効時には告知本文を表示せず直接移動する．検索条件・ハッシュ・ローカルoriginを保持する．追加差分は旧URLの転送・計測禁止テスト2件PASS，Pages build/finalize/verify PASS（70ファイル）．全1,105件PASSはこの告知削除前の版，追加差分2件は告知削除後の検査として区別する．
