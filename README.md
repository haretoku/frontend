...
> **結論：business-researchの要件を，グランドデザインでは利用者体験，backendでは計算・データへ変換し，画面設計と公開データに基づいて実装する．**
...
| 実装コード | 合意済みの画面設計を動作する形で実現する | 未合意の仕様判断 |

上位文書の方針と下位文書の具体指定が矛盾する場合は，上位文書を優先し，必要な判断を上位文書へ反映してから下位文書と実装を更新する．計算式と公開データはbackend，利用者体験とページ責務はグランドデザイン，表示・操作は画面設計を正本とする．
